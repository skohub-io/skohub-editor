        ...
        <li key={item.id}>
          <span
            dangerouslySetInnerHTML={{
              __html: translate(item.prefLabel).replace(
                highlight,
...
const SkohubLookup = (props) => {
  ...
  const schemaLocation = (
    options.url || schema.properties.inScheme.properties.id.enum[0]
  ).replace(/^https?:/, "");

  value &&
    registerSubmitCallback((data) => {
      fetch(value.id.replace(/^http:/, "")).then((response) => {
        ...
        fetch(`/inbox?actor=${actor}`, {
          ...
          .then((res) =>
            alert(
              res.ok ? `Published to ${label}` : `Could not publish to ${label}`
            )
          )
  ...
  // Prettify Me this is ugly, but it works
  const schemaFetch = (error = false) => {
    const headers = {
      Accept: "application/json",
    };
    if (error) {
      return fetch(schemaLocation, { headers: headers });
    } else {
      return fetch(
        schemaLocation.endsWith("index.json")
          ? schemaLocation
          : schemaLocation.endsWith("/")
  ...
  useEffect(() => {
    ...
    fetch(
      schemaLocation.endsWith("index.json")
        ? schemaLocation.replace(/.json?/, ".index")
        : schemaLocation.endsWith("/")
  ...
  return (
    <div
      className={`SkohubLookup ${property || ""} ${
    ...
    >
      ...
      {scheme &&
        (value ? (
          <div
            className="skohubLookupSelectedValue"
